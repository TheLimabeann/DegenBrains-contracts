...
contract DBFarmsLogic {
    ...
    function boost(uint256 _pid, address _account, ERC721 _booster, uint256 _tokenId) public {
        require(msg.sender == masterContract, "not auth");
        PoolInfo memory pool = Master(masterContract).getPool(_pid);
        bool found = false;
        for(uint i = 0; i < pool.boosters.length; i++){
        ...
        require(found, "boost NA with this nft");
        UserBoosters[] memory userBoosters = Master(masterContract).getUserBoosters(_pid, _account);
        require(userBoosters.length < Master(masterContract).getMaxBoost(), "max reached");
        Master(masterContract).updatePool(_pid);
        UserInfo memory user = Master(masterContract).getUser(_pid, _account);
        uint256 pending = user.amount.mul(pool.accPerShare).div(1e18).sub(user.rewardDebt);
        if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPerShare).div(1e18);
        Master(masterContract).setUser(_pid, _account, user);
        _booster.safeTransferFrom(_account, masterContract, _tokenId, "");
        UserBoosters memory _userBoosters = UserBoosters({booster: _booster, tokenId: _tokenId});
        Master(masterContract).addUserBoosters(_pid, _account, _userBoosters);
    }
    function unboost(uint256 _pid, address _account, ERC721 _booster, uint256 _tokenId) public {
        require(msg.sender == masterContract, "not auth");
        UserBoosters[] memory userBoosters = Master(masterContract).getUserBoosters(_pid, _account);
        bool found = false;
        for(uint i = 0; i < userBoosters.length; i++){
            if(userBoosters[i].booster == _booster && userBoosters[i].tokenId == _tokenId) {
                found = true;
                break;
            }
        }
        require(found, "nft not boosted");
        PoolInfo memory pool = Master(masterContract).getPool(_pid);
        UserInfo memory user = Master(masterContract).getUser(_pid, _account);
        uint256 pending = user.amount.mul(pool.accPerShare).div(1e18).sub(user.rewardDebt);
        if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPerShare).div(1e18);
        Master(masterContract).setUser(_pid, _account, user);
        Master(masterContract).recoverNft(_account, _tokenId, _booster);
        UserBoosters memory _userBoosters = UserBoosters({booster: _booster, tokenId: _tokenId});
        Master(masterContract).removeUserBoosters(_pid, _account, _userBoosters);
    }
